...
TinyURL, Twitter, Uber, Netflix, Dropbox, Google Docs, Redis, Kafka, Booking
Engine y Airbnb están en estado `benchmarked`: todos los capítulos-proyecto
planeados tienen diseño, modelo Rust, ejemplo, pruebas, doctests, diagramas y
baseline de benchmark. No están marcados como `reviewed` ni `published` porque
falta revisión humana de contenido, claridad, ortografía, honestidad autoral y
...
Revisión humana de los diez capítulos-proyecto antes de marcar capítulos como
`reviewed` o `published`. Después de esa revisión, el siguiente avance natural
es decidir editorialmente qué capítulo se prepara primero para `academy-web`.
